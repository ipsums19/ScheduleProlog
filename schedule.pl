...
% classToday-C-D     meaning "course C have class on day D"
...
defineClassToday:- course(C), day(D), findall(courseHour-C-D-H, hour(H), Lits), expressOr(classToday-C-D, Lits), fail.
defineClassToday.

%every course can have at most 1hour every day
teachingHours:- course(C), day(D), findall(courseHour-C-D-H, hour(H), Lits), atMost(1, Lits), fail.
%every course have 3 hours per week
teachingHours:- course(C), findall(classToday-C-D, day(D), Lits), exactly(3, Lits), fail.
teachingHours.

%an student can't have two class at the same time
noOverlap:- student(S), student(S,CList), day(D), hour(H), findall(courseHour-C-D-H, member(C,CList), Lits), atMost(1, Lits), fail.
noOverlap.

%every course is given at the same room
sameRoom:- course(C), courseRooms(C,RList), findall(courseRoom-C-R, member(R, RList), Lits), exactly(1, Lits), fail.
%not two courses in the same room,same day, same hour
sameRoom:- room(R), day(D), hour(H), course(C1), course(C2), C1 < C2, courseRooms(C1,Rlist1), courseRooms(C2, Rlist2),
                member(R,Rlist1), member(R, Rlist2), writeClause([\+courseRoom-C1-R,\+courseRoom-C2-R,\+courseHour-C1-D-H,\+courseHour-C2-D-H]), fail. 
sameRoom.

%every student have atMost 3hours per day
atMost3Hours:- student(S), student(S,CList), day(D), findall(classToday-C-D, member(C,CList), Lits), atMost(3, Lits), fail.
atMost3Hours.

atMostXLates(K):- findall(late-D, day(D), Lits), atMost(K,Lits).
...
%%%%%% main:
main:- between(0, 5, K), K_ is 5-K, write('Probando con K = '),
        write(K_), nl, main2(K_), fail.
main:- halt.

main2(K):-  symbolicOutput(1), writeClauses, halt.   % print the clauses in symbolic form and halt
main2(K):-  initClauseGeneration,
